...
struct Commit {
    ...
    Commit() = default;
    explicit Commit(std::chrono::system_clock::time_point _timestamp) : timestamp(_timestamp) {}
};

inline Commit makeInitialCommit() {
    return Commit(std::chrono::system_clock::time_point{});
}
...
[[nodiscard]] inline std::string serialize(const Commit& obj) {
    std::string all;
    all.append(ser::serialize(obj.id));
